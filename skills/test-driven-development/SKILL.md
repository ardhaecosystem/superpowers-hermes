...
metadata:
  hermes:
    tags: [superpowers, tdd, testing, quality, workflow]
    homepage: https://github.com/obra/superpowers
    related_skills: [systematic-debugging, verification-before-completion, writing-plans]
---
...
RED → Write failing test
  → Verify fails correctly (MANDATORY - never skip)
    → wrong failure? → Fix test
    → correct failure? → GREEN
GREEN → Write minimal code to pass
  → Verify passes (MANDATORY)
...
Run your test framework's command for the specific test:
```bash
pytest tests/path/test.py::test_name -v
# or
npm test -- --grep "test_name"
# or
python -m unittest tests.path.test.TestClass.test_name
```
...
Run the same test command again. Confirm:
- Test passes
...
- Over-mocking (mocks everywhere = no real code tested)
- Fragile tests (break on every refactor)
- Slow tests (slow feedback loop discourages running them)
